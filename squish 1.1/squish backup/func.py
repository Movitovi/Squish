...
class game():
    ...
    def __init__(self):
        ...
        # Load game.txt file
        file = open(os.getcwd() + '\\game.txt')
        game_file = file.readlines()
        file.close()
        file = open(os.getcwd() + '\\save.txt')
        ...
        self.objects = objects(game_file, save_file)
    ...
    def update(self):
        ...
        for block in self.objects.blocks:
            pygame.draw.rect(self.display, block.color, [block.pos, block.size])
        for player in self.objects.players:
            if player.alive:
                pygame.draw.rect(self.display, player.color, [player.pos, player.size])
        if self.alive_cnt <= 1:
            # Game over
            self.win_image = self.font.render(self.objects.players[self.alive_ind].name + ' wins!', True, (0, 0, 0))
            self.display.blit(self.win_image, [84, 64])
    ...
    def __init__(self, game_file, save_file):
        ...
        while i < len(game_file):
            if game_file[i] == 'type = player\n':
                # Read player data
                self.players.append(player_object(game_file[i+1:i+11], save_file))
                i += 11
...
class player_object():
    ...
    def set_controls(self):
        ...
        self.controls.append(self.set_key())
