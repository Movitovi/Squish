...
class game():
            ...
            if button.text_image != 0:
                ...
                for action in button.actions:
                    if action[0] == 'input':
                        ...
                        had_input = 1
                if button.text_variable != '':
                    variable_text = ''
                    if button.text_variable == 'resolution':
                        button_text = button.text.format(str(self.resolution[0]) + 'x' + str(self.resolution[1]))
                    had_input = 1
    ...
    def run_level(self):
        ...
        for i in range(0, len(self.players)):
            ...
            if plajer.alive:
                player_cnt += 1
                self.winner_index = i
                plajer.display(lvl, self.su_rects)
            else:
    ...
    def update(self):
        if self.show_fps and not self.menu_controls['page_entrance']:
            ...
            if ((self.mode == 'tron') or (self.page == 'end_tron') or (self.page == 'pause_tron')) and not (self.mode == 'in_game'):
                p = [70 * m, 70 * m]
            else:
                p = [16 * m, 16 * m]
        ...
        if self.su_all:
            ...
            for rect in usu_rects:
                if (self.mode == 'in_game') or (self.mode == 'editor') or (self.mode == 'tron'):
...
class player():
    ...
    def append_su_rects(self, lvl, su_rects):
        if (self.alive and ((self.pos != self.lpos) or (self.size != self.lsize) or (self.color != self.lcolor))) or self.lalive:
