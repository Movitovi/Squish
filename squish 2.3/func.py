...
class game():
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self.do_hat and ((event.value[0] != 0) ^ (event.value[1] != 0)):
                    self.new_controls[self.current_map] = [event.joy, 'hat', event.hat, int(event.value), 0, 0]
                    self.has_mapped = 1
                    self.do_hat = 0
                    break
    ...
    def run_page(self):
        if self.menu_controls['page_entrance']:
            if self.pages[self.page].settings.count('take_screenshot'):
                self.surface_screenshot = pygame.display.get_surface().copy()
            else:
        ...
        for lisk in self.pages[self.page].lists:
            ...
            for i in range((lisk.only_last) * (list_length - 1), list_length):
                ...
                if (lisk.type == 'player') or (lisk.type == 'score'):
                    ...
                    entry_name = entry[1]
                    rank_image = text2img(self.ranks[i], lisk.text_color, lisk.text_font, lisk.text_size, lisk.text_bold)
                    ...
                    score_image = text2img(str(entry[0]).rjust(5), lisk.text_color, lisk.text_font, lisk.text_size, lisk.text_bold)
                    ...
                    rank_image_pos = [lisk.rank_pos[0] - rank_image[1][0] / 2, lisk.rank_pos[1]]
                    entry_surface.blit(rank_image[0], rank_image_pos)
                    score_image = text2img(str(entry).rjust(7), lisk.text_color, lisk.text_font, lisk.text_size, lisk.text_bold)
                    score_image_pos = [lisk.score_pos[0] - score_image[1][0] / 2, lisk.score_pos[1]]
                    entry_surface.blit(score_image[0], score_image_pos)
                if lisk.only_last:
                    i = 0
                if entry_name != '':
                    if lisk.text_limit == None:
                        entry_surface.blit(text2img(entry_name, lisk.text_color, lisk.text_font, lisk.text_size, lisk.text_bold)[0], lisk.text_pos)
                    else:
                        text_surface = pygame.Surface(lisk.text_limit, pygame.SRCALPHA)
                        text_surface.blit(text2img(entry_name, lisk.text_color, lisk.text_font, lisk.text_size, lisk.text_bold)[0], [0, 0])
        ...
        for slider in self.pages[self.page].sliders:
            ...
            if slider.text_variable != '':
                ...
                [slider.text_image, new_text_image_size] = text2img(variable_text, slider.text_color, slider.text_font, slider.text_size, slider.text_bold)
    ...
    def run_level(self):
            ...
            if plajer.alive:
                ...
                if (plajer.name.lower() == 'link') and (plajer.base_color == 0x008800):
                    ...
                    if plajer.controls['ability'][-1]:
                        plajer.spawn_arrow(lvl, self.img_arrow)
                    ...
                    # Spawn new fire
                    if plajer.controls['ability'][-1]:
                        plajer.spawn_fire(lvl, self.img_fire)
                    # Move existing fires
                    kill_fires = []
                    ...
                    # Spawn new zap
                    if plajer.controls['ability'][-1]:
                        plajer.spawn_zap(self.imgs_zap, self.colorkey)
                    # Move existing zaps
                    kill_zaps = []
                    for ii in range(0, len(plajer.zaps)):
                        ...
                        if zap.life > 0:
                            zap.life += -1
                            if zap.facing > 0:
                                p = [plajer.pos[0] + plajer.size[0], plajer.pos[1] + 0.5 * plajer.size[1] - 0.5 * zap.size[1]]
                            else:
                                p = [plajer.pos[0] - zap.size[0], plajer.pos[1] + 0.5 * plajer.size[1] - 0.5 * zap.size[1]]
                            for iii in range(0, len(self.players)):
                                if i != iii:
                                    plaier = self.players[iii]
                                    if plaier.alive:
                                        overlap = rect_overlap(plaier.pos, plaier.size, [0, 0], p, zap.size, lwl = lvl)
                                        if magnitude(overlap) >= plajer.penetration_depth:
                                            plaier.ability_death(plajer, self.shield_color, self.death_animations, lvl)
                        else:
                            kill_zaps.append(ii)
            ...
            if plajer.alive:
                if (plajer.name.lower() == 'pikachu') and (plajer.base_color == 0xffff00):
                    for ii in range(0, len(plajer.zaps)):
                        zap = plajer.zaps[ii]
                        if zap.life > 0:
                            lvl.surface.blit(zap.get_image(), p)
        ...
        if (player_cnt <= 1) and (len(self.death_animations) == 0):
    ...
    def do_actions(self, actions, index = -1):
        for action in actions:
            if action[0] == 'quit':
            ...
            elif action[0] == 'toggle_fullscreen':
            ...
            elif action[0] == 'update_controllers': # [Redact] this isn'y being used
                self.reset_joysticks()
                ...
                self.mode = 'tron'
                self.tron_game.reset(self.players)
    ...
    def editor_display_level(self, show_extra = False):
        ...
        if show_extra:
            ...
            if self.placing_block:
                new_block = point2size(self.editor_block_pos, self.mouse_pos, self.editor_rounding)
    ...
    def get_snake_inputs(self):
        self.snake_game.get_inputs(self.players, self.controllers, self.joysticks, self.joystick_threshold)

    def play_snake(self):
        if self.snake_game.alive:
            if self.snake_game.counter >= self.tick_speed / self.snake_game.tick_speed:
                self.snake_game.update_snake()
            ...
            self.snake_game.display_death(self.surface)
    ...
    def play_scythe(self):
        if self.scythe_game.running:
            if not self.scythe_game.game_over:
                self.scythe_game.update_scythe()
            elif len(self.scythe_game.death_particles) <= 0:
                self.scythe_game.running = 0
            self.scythe_game.display_scythe(self.surface)
        else:
            self.secret = self.secrets[random.randint(0, len(self.secrets) - 1)]
    ...
    def show_secret(self):
        [img, s] = text2img(self.secret, 0, 'couriernew', 32, 1)
    ...
    def update_window_size(self, update_scales = 1):
        ...
        if update_scales:
            self.screen_scales = [self.display_size[0] / self.display_info.current_w, self.display_size[1] / self.display_info.current_h]
    ...
    def update(self):
        if self.show_fps and not self.menu_controls['page_entrance']:
            if (self.mode == 'tron') or (self.page == 'end_tron') or (self.page == 'pause_tron'):
                p = [70, 70]
            else:
                p = [16, 16]
            self.surface.blit(text2img(str(0.1 * round(10 * self.clock.get_fps()))[0:4], 0, 'couriernew', 32, 1)[0], p)
...
class controller():
    ...
    def check_inputs(self, menu_controls, menu_input_delay_index, is_first_frame, dir_override = 0, select_override = 0):
...
class player():
    def __init__(self, existing_players):
        ...
        possible_controllers = len(existing_players) * controls_length # <- Change this value to match number of controls a player has
    ...
    # [Redact] This isn't being used at all
                ...
                if not (plajer.boosts['phase'] and plajer.controls['ability'][-1]):
                    if check_collision_direction(self.lpos, self.size, self.vel, plajer.pos, plajer.size, lvl):
                        if compare_wrapped(self.lpos, self.size, plajer.pos, plajer.size, lvl, 0):
                            ...
                            if compare_wrapped(self.lpos, self.size, plajer.pos, plajer.size, lvl, 1):
    ...
    def display(self, lvl):
        ...
        if (self.name.lower() == 'yoshi') and (self.base_color == 0x00aa00):
            if not self.shield_broken and self.controls['shield'][-1]:
                ...
                p = [0, 0.15 * self.size[1]]
                s = [0.4 * self.size[0], 0.4 * self.size[1]]
                pygame.draw.rect(self.surface, 0x00aa00, [p, s])
                p = [0.6 * self.size[0], 0]
                s = [0.4 * self.size[0], 0.3 * self.size[1]]
                pygame.draw.rect(self.surface, 0x00aa00, [p, s])
                p = [0.5 * self.size[0], 0.45 * self.size[1]]
                s = [0.5 * self.size[0], 0.5 * self.size[1]]
                pygame.draw.rect(self.surface, 0x00aa00, [p, s])
        if self.boosts['kill']:
        ...
        if self.boosts['invincible']:
            pygame.draw.rect(self.surface, self.invincible_color, [[0, 0], [0.15 * self.base_size[0], self.base_size[1]]])
            pygame.draw.rect(self.surface, self.invincible_color, [[0, 0], [self.base_size[0], 0.15 * self.base_size[1]]])
            pygame.draw.rect(self.surface, self.invincible_color, [[0.85 * self.base_size[0], 0], [0.15 * self.base_size[0], self.base_size[1]]])
            pygame.draw.rect(self.surface, self.invincible_color, [[0, 0.85 * self.base_size[0]], [self.base_size[0], 0.15 * self.base_size[1]]])
    ...
    def spawn_fire(self, lvl, image):
        if self.shield_broken or not self.controls['shield'][-1]:
            if self.fire_counter <= 0:
                ...
                self.fires.append(fire(p, s, v, image))
...
class fire():
    def __init__(self, pos, size, vel, image):
        self.size = [size, size]
        self.pos = [pos[0] - 0.5 * self.size[0], pos[1] - 0.5 * self.size[1]]
        self.vel = vel[:]
        self.life = 20
...
class death_animation():
    def __init__(self, pos, size, life = 10, colors = [0xffffff, 0x000000, 0x000000]):
        ...
        self.rects = [[pos[:], [0, 0]], [pos[:], [0, 0]], [pos[:], [0, 0]], [pos[:], [0, 0]]]
    ...
    def get_rects(self):
        ...
        for i in range(0, 2):
            for ii in range(0, 2):
                ...
                s[1] += self.delta[1]
                r = [p, s]
                return_rects.append([[p[0] - 0.5 * s[0], p[1] - 0.5 * s[1]], s])
        self.life += -1
        self.color = self.colors[self.life % self.color_count]
        return return_rects
...
class snake_game():
    ...
    def get_inputs(self, players, controllers, joysticks, joystick_threshold):
        self.counter += 1
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                self.quit_squish = 1
    ...
    def change_grass(self):
        self.grass[random.randint(0, len(self.grass) - 1)] = [random.randint(0, self.size[0] - 1), random.randint(0, self.size[1] - 1)]
    ...
    def display_death(self, display):
        display.fill(self.background_color)
        self.snake_surface.fill(self.background_color)
        for pos in self.grass:
            self.snake_surface.set_at(pos, self.grass_color)
...
class scythe_game():
    ...
    def change_flowers(self):
        self.surface_flowers = self.surface_fence_upper.copy()
        flower = [round(random.randint(0, self.size[1] - 1) + 0.5 * self.size_image[0]), round(random.randint(0, self.size[0] - 1) + 0.5 * self.size_image[1]), random.randint(1, 2)]
        self.flowers[random.randint(0, len(self.flowers) - 1)] = flower
        self.flowers.sort()
        for flower in self.flowers:
            if flower[2] == 1:
                image = self.img_flower_1
    ...
    def display_scythe(self, display):
        ...
        [img, s] = text2img(str(round(self.score)), 0, 'couriernew', 48, 1)
        ...
        display.blit(img, [display_size[0] - 5.17 * s[1] - s[0], 0.75 * s[1]])
...
class tron_game():
    ...
    def reset(self, players):
        ...
        for i in range(0, len(players)):
            plajer = players[i]
            self.players.append(tron_player(plajer, next_pos, next_din, self.delta_din, self.speed, self.size_player[0], self.size_trail, self.length, self.trail_rate, self.trail_accent, self.colorkey))
            if i < len(players) - 1:
...
class tron_player():
    def __init__(self, plajer, pos, din, delta_din, speed, size, trail_size, length, trail_rate, trail_accent, colorkey):
        self.player = plajer
        self.opos = pos[:]
        self.odin = din
...
def text2img(text, color, font, font_size, is_bold):
    font = pygame.font.SysFont(font, round(font_size), is_bold)
...
def compare_wrapped(p1, s1, p2, s2, lvl, i, inclusive = 0, precision = 10000000):
    if inclusive:
        if round(precision * (p1[i] + s1[i])) / precision >= p2[i]:
            if round(precision * (p2[i] + s2[i])) / precision >= p1[i]:
                return 1
        if (round(precision * (p1[i] + s1[i])) / precision >= lvl.size[i]) or (round(precision * (p2[i] + s2[i])) / precision >= lvl.size[i]):
            if ((i == 0) and lvl.horizontal_looping) or ((i == 1) and lvl.vertical_looping):
                if round(precision * (p1[i] + s1[i])) / precision - lvl.size[i] >= p2[i]:
                    if round(precision * (p2[i] + s2[i])) / precision >= p1[i] - lvl.size[i]:
                        return 1
                if round(precision * (p1[i] + s1[i])) / precision + lvl.size[i] >= p2[i]:
                    if round(precision * (p2[i] + s2[i])) / precision >= p1[i] + lvl.size[i]:
                        return 1
