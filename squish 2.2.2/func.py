...
class game():
    def __init__(self):
        ...
        self.display_size = [0.9 * self.display_info.current_w, 0.9 * self.display_info.current_h]
        self.font_multiplier = 0.5 * (self.display_size[0] / 1920 + self.display_size[1] / 1080)
    ...
    def load_pages(self):
        ...
        for pg in os.listdir(pgd):
            ...
            self.pages[pg.replace('.txt', '')] = page(file.readlines(), self.display_size, self.font_multiplier)
            file.close()
    ...
    def run_page(self):
        ...
        for block in self.pages[self.page].blocks:
            ...
            if block.text_variable != '':
                ...
                [block.text_image, new_text_image_size] = text2img(variable_text, block.text_color, block.text_font, block.text_size * self.font_multiplier, block.text_bold)
            if block.color != -1:
        ...
        self.cursor_on_list = 0
        for lisk in self.pages[self.page].lists:
            list_length = len(self.players) * ((lisk.type == 'player') or (lisk.type == 'score')) + len(self.levels) * (lisk.type == 'level') + len(self.snake_scores) * (lisk.type == 'snake_score')
            ...
            for i in range((lisk.only_last) * (list_length - 1), list_length):
                ...
                if (lisk.type == 'player') or (lisk.type == 'score'):
                    ...
                    if lisk.type == 'score':
                        score_image = text2img(str(entry.wins), lisk.text_color, lisk.text_font, lisk.text_size * self.font_multiplier, lisk.text_bold)
                        score_image_pos = [lisk.wins_pos[0] - score_image[1][0] / 2, lisk.wins_pos[1]]
                        entry_surface.blit(score_image[0], score_image_pos)
                        score_image = text2img(str(entry.kills), lisk.text_color, lisk.text_font, lisk.text_size * self.font_multiplier, lisk.text_bold)
                    ...
                    entry_name = entry[1]
                    rank_image = text2img(self.ranks[i], lisk.text_color, lisk.text_font, lisk.text_size * self.font_multiplier, lisk.text_bold)
                    ...
                    score_image = text2img(str(entry[0]).rjust(5), lisk.text_color, lisk.text_font, lisk.text_size * self.font_multiplier, lisk.text_bold)
                    score_image_pos = [lisk.score_pos[0] - score_image[1][0] / 2, lisk.score_pos[1]]
                    entry_surface.blit(score_image[0], score_image_pos)
                if lisk.only_last:
                    i = 0
                if lisk.text_limit == None:
                    entry_surface.blit(text2img(entry_name, lisk.text_color, lisk.text_font, lisk.text_size * self.font_multiplier, lisk.text_bold)[0], lisk.text_pos)
                else:
                    text_surface = pygame.Surface(lisk.text_limit, pygame.SRCALPHA)
                    text_surface.blit(text2img(entry_name, lisk.text_color, lisk.text_font, lisk.text_size * self.font_multiplier, lisk.text_bold)[0], [0, 0])
        ...
        for button in self.pages[self.page].buttons:
            ...
            if button.text_image != 0:
                ...
                if had_input:
                    input_surface.blit(text2img(button.text + button.text_input, button.text_color, button.text_font, button.text_size * self.font_multiplier, button.text_bold)[0], [0, 0])
                else:
        ...
        for i in range(0, len(self.players)):
            ...
            if plajer.alive:
                ...
                for ii in range(0, 2):
                    ...
                    if plajer.vel[ii] < -plajer.vel_terminal[ii]:
                        plajer.vel[ii] = -plajer.vel_terminal[ii]
                if plajer.controls['jump'][-1] and (plajer.on_ground or plajer.on_player):
    ...
    def do_actions(self, actions, index = -1):
        for action in actions:
            if action[0] == 'quit':
                ...
                self.mode = 'snake'
                self.snake_game.restart(self.display_size, self.players[0].base_color, self.players[0].name)
            elif action[0] == 'play_scythe':
                self.mode = 'scythe'
                self.scythe_game.restart(self.display_size)
    ...
    def play_snake(self):
        if self.snake_game.alive:
            self.snake_game.get_snake_inputs(self.players, self.controllers, self.joysticks, self.joystick_threshold)
    ...
    def load_secrets(self):
        file = open(os.path.join(self.cwd, 'secrets.txt'), encoding = 'utf-8')
    ...
    def update(self):
        ...
        if self.show_fps:
            self.display.blit(text2img(str(0.1 * round(10 * self.clock.get_fps()))[0:4], 0, 'couriernew', 32 * self.font_multiplier, 1)[0], [16, 16])
        # NOTE: debugging display here
        if self.show_debug:
            if len(self.players):
                p = self.players[2]
                self.overlaps = ['squish_width: ' + str(p.squish_width), 'width: ' + str(p.size[0]), 'pos: ' + str(p.pos)]
            for i in range(0, len(self.overlaps)):
                overlap = self.overlaps[i]
                self.display.blit(text2img(str(overlap), 0, 'couriernew', 32 * self.font_multiplier, 1)[0], [16, 80 + 32 * i])
...
class page():
    def __init__(self, file, display_size, font_multiplier):
        self.blocks = []
        self.buttons = []
        self.controls = []
        self.lists = []
        ...
        for line in file:
            ...
            if properky == 'block':
                ...
                [objekt.text_image, objekt.text_image_size] = text2img(objekt.text, objekt.text_color, objekt.text_font, objekt.text_size * font_multiplier, objekt.text_bold)
...
class death_animation():
    def __init__(self, pos, size):
        ...
        self.size = size
        self.life = 10
        self.colors = [0xffffff, 0x000000, 0x000000]
...
class snake_game():
    ...
    def restart(self, display_size, snake_head, name):
        pygame.mouse.set_visible(0)
        self.quit_squish = 0
        self.alive = 1
        self.score = 0
        ...
        self.mult = round((display_size[0] / self.size[0] + display_size[1] / self.size[1]) / 2)
        self.snake = [[5, 4]]
        valid_spots = []
    ...
    def get_snake_inputs(self, players, controllers, joysticks, joystick_threshold):
        self.counter += 1
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                self.quit_squish = 1
    ...
    def display_snake(self, display):
        ...
        for pos in self.grass:
            pygame.draw.rect(display, self.grass_color, [[pos[0] * self.mult, pos[1] * self.mult], [self.mult, self.mult]])
        pygame.draw.rect(display, self.apple_color, [[self.apple[0] * self.mult, self.apple[1] * self.mult], [self.mult, self.mult]])
        for i in range(0, len(self.snake)):
            pos = self.snake[i]
            #if (((self.index - i) % len(self.snake))):# % 2): This with an else statement will make the snake stripy if you want
            pygame.draw.rect(display, self.snake_color, [[pos[0] * self.mult, pos[1] * self.mult], [self.mult, self.mult]])
        pos = self.snake[self.index]
        pygame.draw.rect(display, self.snake_head, [[self.snake[self.index][0] * self.mult, self.snake[self.index][1] * self.mult], [self.mult, self.mult]])
    ...
    def update_death(self):
        if self.death_particle == None:
            if self.death_wait_cnt < self.death_wait_time:
                ...
                p = [(pos[0] + 0.5) * self.mult, (pos[1] + 0.5) * self.mult]
    ...
    def display_death(self, display):
        ...
        if self.death_particle != None:
            for rect in self.death_particle.get_rects():
                pygame.draw.rect(display, self.death_particle.color, rect)
...
class scythe_game():
    ...
    def restart(self, display_size):
        pass

    def get_scythe_inputs(self, controllers, joysticks, joystick_threshold):
        pass
...
def compare_wrapped(p1, s1, p2, s2, lvl, i, inclusive = 0):
    if inclusive:
        if p1[i] + s1[i] >= p2[i]:
            if p2[i] + s2[i] >= p1[i]:
                return 1
        if (p1[i] + s1[i] >= lvl.size[i]) or (p2[i] + s2[i] >= lvl.size[i]):
