...
class game():
    ...
    def run_level(self):
        ...
        # If pushing against each other maintain present velocity but converge to desired total velocity
...
class player():
    ...
    def player_collision(self, plajer, lvl): #, player_indeces):
        #plajer = players[player_indeces[-1]]
        k = self.vel[0] < self.vel[1]
        hit_player = 0
        for i in [k, not k]:
            if self.vel[i]:# and plajer.alive:
                if compare_wrapped(self.pos, self.size, plajer.pos, plajer.size, lvl, not i):#This spot was using self.lpos and I think it shouldn't
