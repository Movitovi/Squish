    ...
    def run_level(self):
        ...
        for i in range(0, len(self.players)):
            ...
            if plajer.alive:
                ...
                plajer.pos = result[0]
                plajer.update_vel(lvl, result[1])
                ...
                if action[1] == 'level':
                    ...
                    while 1:
                        ...
                        if not has_key:
                            ...
                            break
                        key = key + '-'
                    self.levels[key].export_level_file()
                    self.level = key
                elif action[1] == 'player':
        ...
        if self.text_input_action != -1:
            ...
            if self.text_input_action[1] == 'level':
                if self.text_input_action[2] == 'name':
                    ...
                    if text == '':
                        self.levels[self.level].side_wall_depth = 0
                    else:
                        self.levels[self.level].side_wall_depth = float(text)
...
class player():
    ...
    def update_vel(self, lvl, wrapped):
        ...
        for i in range(0, 2):
            if wrapped[i] == -1:
                # wrapped from top to bottom or left to right
                self.vel[i] -= lvl.size[i]
            elif wrapped[i] == 1:
                # wrapped from bottom to top of right to left
                self.vel[i] += lvl.size[i]
        '''
...
def rect_overlap(p1, s1, v1, p2, s2, v2 = [0, 0], inclusive = 1):
    overlap = [0, 0]
    for i in range(0, 2):
        if inclusive:
            if p1[i] + s1[i] >= p2[i]:
                if p1[i] <= p2[i] + s2[i]:
                    if p1[i] - v1[i] <= p2[i] - v2[i]:
...
def find_wrapped_point(point, size, lvl, i = -1):
    ...
    if i == -1:
        if lvl.horizontal_looping:
            ...
            elif point[0] > lvl.size + size[0] * lvl.side_wall_depth * 0.01:
                point[0] = lvl.size + size[0] * lvl.side_wall_depth * 0.01
        if lvl.vertical_looping:
            if lvl.size[1] != 0:
                if point[1] < -size[1]:
                    wrapped[1] = -1
        ...
        if lvl.horizontal_looping:
            if lvl.size[0] != 0:
                ...
                elif point > lvl.size + size[0] * lvl.side_wall_depth * 0.01:
                    point = lvl.size + size[0] * lvl.side_wall_depth * 0.01
    elif i == 1:
        if lvl.vertical_looping:
            if lvl.size[1] != 0:
                if point < 0:
