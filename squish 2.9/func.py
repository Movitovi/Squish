...
class game():
    ...
    def load_sprites(self):
        # This is the colorkey used as transparent color
    ...
    def reset_menu_navigation(self):
        ...
        self.menu_controls = {'up': 0,
                              ...
                              'page_entrance': 0,
                              'continual': 1}
        # This handles delaying the repeat of navigation while holding a key
    ...
    def map_controls(self):
        # Check if an event has been mapped
        if not self.mapping:
    ...
    def map_event(self):
        # This is for mapping a new control to a player
        no_joy = 1
        no_hat = 1
    ...
    def find_list_buttons(self):
        # This recreates the button array used for menu navigation
        # It is necessary for lists that are selectable and might change size
    ...
    def run_page(self):
        # Update the whole screen on the first frame
        if self.mode_entrance:
            self.mode_entrance = 0
            self.su_all = 1
        
        # Manage anything that should happen upon page entrance
        ...
        # This is for pausing page navigation on entrance, like for the pause or end screen pages
        if self.page_cnt > 0:
            self.reset_menu_navigation()
            self.mouse_moved = 0
            self.page_cnt -= 1
        ...
        for slider in self.pages[self.page].sliders:
            if self.cursor == slider.button_pos:
                ...
                if (self.mouse_lmb and not self.mouse_last_lmb) or (self.mouse_lmb and self.slider_latch):
        ...
        # Find previous and new buttons the cursor is on for the update rectangles
        for button in self.pages[self.page].buttons:
        ...
        # Show secrets if the score was high enough
            ...
            if not plajer.is_cpu:
                for control_key in plajer.controls:
                    ...
                    if len(self.controllers) > 0:
                        # This part (intentionally) causes 1 controller to control multiple players
    ...
    def run_level(self):
        # Display the whole screen on mode entrance
        if self.mode_entrance:
            self.mode_entrance = 0
            self.su_all = 1
        ...
        # Check if players kill or are killed
        for i in range(0, len(self.players)):
        ...
        # Handle player horizontal collision
        for i in range(0, len(self.players)):
            plajer = self.players[i]
            if plajer.alive:
                plajer.check_player_horizontal_collision(self.players, lvl, i)

        # Let player pushing propogate. Necessary for proper squishing
        for i in range(0, len(self.players)):
            plajer = self.players[i]
            if plajer.alive:
        ...
        # Update the squish state of a player
        for i in range(0, len(self.players)):
            plajer = self.players[i]
            if plajer.alive:
    ...
    def initialize_boosts(self):
        ...
        # The boost_types values are the number of ticks the boost with last for
    ...
    def do_actions(self, actions, index = -1):
        # This function manages many of the important actions from the pages and other parts of the program
        # It allows pages to be a little more general in how they work
        for action in actions:
            if action[0] == 'quit':
                ...
                if action[1] == 'level':
                    ...
                    self.players = []
            elif action[0] == 'reinsert_player':
                self.reinsert_player()
    ...
    def apply_text_input(self, text):
        # This just applies input text to the variable tied with a button or page object
        if self.text_input_action != -1:
    ...
    def copy_player(self, index):
        self.reinsert_player_index = index
        plajer = self.players[index]
        new_player = player(self.players)
        new_player.name = plajer.name
        new_player.base_color = plajer.base_color
        new_player.color = plajer.color
        new_player.lcolor = plajer.lcolor
        new_player.name = plajer.name
        new_player.name = plajer.name
        new_player.name = plajer.name

        new_player.kills = plajer.kills
        new_player.wins = plajer.wins
        new_player.team = plajer.team

        for control_key in plajer.controls:
            new_player.controls[control_key] = plajer.controls[control_key][:]
        
        new_player.is_cpu = plajer.is_cpu
        
        self.players.append(new_player)

    def reinsert_player(self):
        self.players[self.reinsert_player_index] = self.players.pop()

    def check_teams(self):
        # This just keeps the player's team within the number of teams
        for plajer in self.players:
    ...
    def check_player_team_event(self, event):
        # This takes a page navigation input from the keyboard,
        # if it is a player control tries to change the player's team.
        # If the player's team changes then do not use the page navigation input
        found_team = 0
        for plajer in self.players:
            lteam = plajer.team
            for control_key in plajer.controls:
                ...
                if control[0] == -1:
                    if event.key == control[1]:
                        if plajer.team != 0:
                            ...
                            if plajer.team > self.team_count:
                                plajer.team = 0
            if plajer.team != lteam:
                found_team = 1
        return found_team
    
    def check_player_team_joystick(self, control_type, control, i, si):
        # This is the same as check_player_team_event() but for controller inputs
        found_team = 0
        for plajer in self.players:
            lteam = plajer.team
    ...
    def distribute_players(self):
        # This distributes unteamed players to teams starting with the smallest teams
        # If two lowest teams have the same number of players then pick one at random
    ...
    def get_editor_inputs(self):
        ...
        for kontroller in self.controllers:
            kontroller.check_inputs(self, dir_override = 1, select_override = eraser)
        self.menu_controls['page_entrance'] = 0
    ...
    def run_editor(self):
        ...
        # Handle cursor motion not from the mouse
        if self.menu_controls['up']:
        ...
        if (self.mouse_lmb and not self.mouse_last_lmb) or self.menu_controls['select']:
            if self.tool == 'block':
                ...
                if self.new_player_pos[1] + lvl.player_size[1] > 0:
                    ...
                    if self.editor_mirroring:
                        lvl.player_poses.append([round(lvl.size[0] - self.new_player_pos[0] - lvl.player_size[0]), self.new_player_pos[1]])
            elif self.tool == 'edit':
        ...
        # Update the whole screen each time the mouse is clicked
        # This is not optimal but the performance gain really isn't really worth the time
        if self.mouse_lmb or self.menu_controls['select']:
            self.su_all = 1
    ...
    def load_secrets(self):
        # The secrets are kept as a single shift of all character bytes
        # This unshifts them and puts them into a list
    ...
    def play_snake(self):
        ...
        if self.snake_game.alive:
            if self.snake_game.counter >= self.tick_speed / self.snake_game.tick_speed:
                # Only update the snake every few frames
            ...
            if (self.snake_game.counter == 0) or (self.snake_game.grass_change_counter == 0):
                # Display the snake if grass has changed or the snake has changed
                self.snake_game.display_snake(self.surface)
                self.surface_screenshot = self.surface.copy()
            else:
                # Otherwise only display a screenshot
            ...
            if self.snake_game.ask_for_quit():
                # Only quit once the death animations are done
    ...
    def update(self):
        if self.show_fps and not self.menu_controls['page_entrance']:
            # This is the fps stuff. Don't show on page entrance in case a screenshot is needed
        ...
        if self.su_all:
            ...
            # Only update some of the screen
            ssu_rects = []
...
class controller():
    def __init__(self, joystick, joystick_threshold):
        ...
        # Each value should be [new_value, old_value, axis_direction, input_sources]
    ...
    def check_inputs(self, game, dir_override = 0, select_override = 0, si = None):
        ...
        for control in self.controls:
            ...
            # This is to remove the flick effect when you let go of a joystick
    ...
    # The check_snake_inputs() and check_scythe_inputs() are both close
    # enough to check_inputs() to not be their own functions entirely.
    def check_snake_inputs(self, new_dir):
        # This is just like check_inputs() but with a few changes
        for control in self.controls:
            self.controls[control][1] = self.controls[control][0]
            self.controls[control][0] = 0
            for i in range(3, len(self.controls[control])):
...
class player():
    def __init__(self, existing_players):
        ...
        # Figure out the lowest controller number that isn't in use already
        # Use that as this player's default controller
        ...
        # Controller input entries should be [controller_number, type, id, direction, last_value, value]
        # Keyboard   input entries should be [-1, key, last_value, value]
    ...
    def pos2(self):
        # This is the far (bottom right) corner
    ...
    def reset_cpu_variables(self):
        self.enemy_count = 0
        # Target inputs are (parent, weight, type, direction, target, lvl = None)
    ...
    def expand(self, lvl, squish_delta, is_a_recurse = 0):
        # This is like the squish function but for dead players
        # Because they can't be squished, they can only expand
        if not is_a_recurse:
            self.extend_image = 0
        if not (self.boosts['phase'] and self.controls['ability'][-1]):
            if (is_a_recurse <= 1) and (not (self.controls['shield'][-1] and not self.shield_broken)):
    ...
    def update_vel(self, lvl):
        # This assumes any movement larger that half the screen was a screen wrap, if wrapping in that direction
    ...
    def check_squished(self, lvl, kill_width, players, death_animations):
        # Check if this player got squished to death
    ...
    def check_kill(self, lvl, players, kill_width, shield_color, player_index, death_animations):
        # Check if this player killed any other player by being on top and within the appropriate squish width
        if not (self.boosts['phase'] and self.controls['ability'][-1]):
    ...
    def ability_death(self, plajer, shield_color, death_animations, lvl):
        # This gives the kill of this player to plajer if appropriate
        if not (self.boosts['phase'] and self.controls['ability'][-1]):
            if not self.boosts['invincible']:
    ...
    def check_ground(self, lvl, players, player_index):
        # This checks if the player is standing on a block and is allowed to jump
    ...
    def display(self, lvl, su_rects, team_colors, do_teams):
        # This renders the player as appropriate
        ...
        if self.boosts['invincible']:
            # Display the invincibility shield
            self.surface.blit(self.invincibility_surface, [0, 0])
        if (self.name.lower() == 'game and watch') and (self.base_color == 0x000000):
            # This is where game and watch is all pixely
            # It does latch to the correct position in one direction at a time
            # if the player stays still in that direction for long enough
            real_pos = self.pos[:]
            real_lpos = self.lpos[:]
    ...
    def append_su_rects(self, lvl, su_rects):
        # This scales the update rectangles appropriately
...
class target():
    def __init__(self, parent, weight, tipe, din, target, lvl = None):
        # Parent is the player that this target belongs to
        self.parent = parent
        self.weight = weight
        self.ticks = 0
        self.type = tipe
...
class level():
    ...
    def rescale(self, refrence = [1920, 1080]):
        # This just resizes the level
        # It does not resize or reposition the blocks or players in the level
    ...
    def duplicate(self, i):
        ...
        for block in old_blocks:
            ...
            new_block.pos[i] = round(self.size[i] - new_block.pos[i] - new_block.size[i])
            already_exists = 0
        ...
        for player_pos in old_player_poses:
            ...
            new_player_pos[i] = round(self.size[i] - new_player_pos[i] - self.player_size[i])
            already_exists = 0
...
class snake_game():
    def __init__(self, sound):
        # Move the snake 4 times per second
        self.tick_speed = 4
    ...
    def update_death(self):
        # Start from the tail and go towards the head making one death animation at a time
...
class scythe_game():
    ...
    def update_scythe(self, surface_size, su_rects):
        ...
        # Change a flower every now and again
        ...
        # Spawn wolves randomly. They spawn faster as there are less sheep
...
class scythe_sheep():
    def __init__(self, image_right, image_left, xmin, xmax, ymin, ymax):
        ...
        self.speed = 0.1
        self.dir = [0, 0]
        # These are the possible directions a sheep will go with their respective weights
    ...
    def bob(self):
        # This is only visual. It doesn't effect the actualy position
...
class scythe_wolf():
    ...
    def find_spawn_point(self, sheep, cnt):
        # Pick a bunch of random spots to spawn and choose the one farthest from all the sheep
        if len(sheep):
    ...
    def update(self, sheep, player_pos):
        ...
        if (distance(player_pos, self.pos) <= self.flee_radius) and (-1 / (len(sheep) + 0.001) + 1 > random.random()):
            # When the player is close and there are still many sheep to eat run from the player, but not always
...
def rect_overlap(p1, s1, v1, p2, s2, v2 = [0, 0], inclusive = 1, override = 0, lwl = None):
    # Check if two rectangles overlap and return the amount of that overlap
    if lwl:
        overlaps = []
        grid = [[-1, -1], [0, -1], [1, -1],
                [-1, 0], [0, 0], [1, 0],
...
def find_wrapped_point(point, size, lvl, i = -1):
    # Wrap a point in a wrapping level if needed
    # Otherwise hit the side walls if needed
    # Return the wrapped point and wether or not it did wrap
    wrapped = [0, 0]
    if i == -1:
        # This means wrap in both directions
        if lvl.horizontal_looping:
            if lvl.size[0] != 0:
                if point[0] < 0:
                    wrapped[0] = -1
        ...
        # This means wrap only horizontally
        if lvl.horizontal_looping:
            if lvl.size[0] != 0:
                if point < 0:
                    wrapped[0] = -1
        ...
        # This means wrap only vertically
        if lvl.vertical_looping:
            if lvl.size[1] != 0:
                if point < 0:
                    wrapped[1] = -1
...
def check_collision_direction(p1, s1, v1, p2, s2, lvl):
    # Check which direction the collision occured in
    if v1[1] == 0:
        return 0
    elif v1[1] > 0:
...
def point2size(p1, p2, rounding = 0):
    # This converts a rectangle from 2 points into 1 point and a size
    if rounding:
...
def has_greater_magnitude(given_list, value):
    # Returns 1 if the value have a greater magnitude than everything in the list
    if len(given_list) == 0:
        return 1
...
def biggest_vector(vectors):
    # Returns the biggest vector of the list
    if len(vectors) == 0:
        return [0, 0]
    index = 0
    mak = 0
...
def keep_within(input_list, limit1, limit2):
    # This binds each value in the input_list within the two limits
    if limit1 >= limit2:
        upper_limit = limit1
        lower_limit = limit2
    else:
...
def shift_chars(input_string, shift_amount):
    # This does a character byte shift thing
...
def distance(vector_1, vector_2, lvl = None):
    # Simply finds the distance between two points
    total = 0
    if lvl:
...
def simple_vector_angle(vector):
    # This finds the angle of each vector for tron
    if vector[0] == -1:
        if vector[1] == -1:
            return 225
        if vector[1] == 0:
...
def unbiased_smallest(input_list):
    # This finds all the smallest values in the input_list and returns the index of one at random
    if len(input_list):
        smallest = []
        value = None
        for i in range(0, len(input_list)):
